...
# 0_2. 전역변수 및 상대 경로 지정
#--------------------------------------------------------------
roadimg_folder_src = 'C:/Users/USER/Desktop/HotSix/deeplearning/result_testimg_01'
roadimg_src= 'C:/Users/USER/Desktop/HotSix/deeplearning/result_testimg_01/img/'
roadimg_name = 'testimg_ca.jpg'
...
#--------------------------------------------------------------
# 각 러닝된 모델 불러오기
model_vegetable = load_model('test_03.h5')
...
output = model_vegetable.predict_generator(test_generator)

# 학습 모델이 판단한 이미지의 판단 결과 확률 임시 저장 변수
#learning_pre_result = 0
...
if vegetable_classnumber == 0:
    ...
    result_output = vegetable_model_class[0].predict_generator(test_generator)
    vegetable_classnumber = find_index(result_output)
    result_class = ["뿌리 혹병", "배추 역병", "무름병"]
    '''
    for i in range(len(output[0])):
        if learning_pre_result < result_output[0][i]:
            learning_pre_result = result_output[0][i]
            vegetable_classnumber = i
            '''
# 1번 인덱스인 고추의 질병 종류 판단
elif vegetable_classnumber == 1:
    result_output = vegetable_model_class[1].predict_generator(test_generator)
    vegetable_classnumber = find_index(result_output)
...
# 2번 인덱스인 깨의 질병 종류 판단
    ...
    result_output = vegetable_model_class[2].predict_generator(test_generator)
    vegetable_classnumber = find_index(result_output)
    result_class = []
    '''
    for i in range(len(output[0])):
...
print("%s: %.2f%%" %(result_class[vegetable_classnumber], result_output[0][vegetable_classnumber]*100))
